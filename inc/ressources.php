

<article class="fifth-section change-color" id="fifth">
<div id="secondaryFifth">
   <!--  <h2 class="gradient">ressources</h2> -->
    <h2 class="buttons gradient">
        ressources
    </h2> 
        
        
    <p class="h2-subtitle">Nous vous livrons ici des ressources satellites à l’installation : <br>fiches pédagogiques, frise chronologique, mappemonde, jeux interstellaires, références, … <br><br><span class="gradient">Contemplez, jouez, imaginez, expérimentez !</span> 
    </p>

    <div class="container">
        <section class="fifth-section-content">
            <div class="fifth-section-content-element">
                <div class="padlet-embed">
                    <iframe src="https://padlet.com/embed/zblxmof8j0ulx225" frameborder="0"
                    allow="camera;microphone;geolocation"></iframe>
                </div>
            </div>


        </section>
    </div>
    </div>
</article>
